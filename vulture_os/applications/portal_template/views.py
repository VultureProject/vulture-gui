...
#                 httpchk_headers_dict[header.get('check_header_name')] = header.get('value')
#
...
#             backend.configuration = backend.generate_conf(header_list=header_objs)
#             """ Save the conf on disk, and test-it with haproxy -c """
#             logger.debug("Writing/Testing conf of frontend '{}'".format(backend.name))
#             backend.test_conf()
...
#             backend.save()
#             logger.info("Backend '{}' (id={}) saved in MongoDB.".format(backend.name, backend.id))
#
...
#                     logger.info("HTTP Headers {} associated to Frontend {}".format(header, backend))
#
...
#             backend.save_conf()
#             logger.info("Write conf of backend '{}' asked on cluster".format(backend.name))
#
