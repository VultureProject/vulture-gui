...
class Cluster(models.Model):
    ...
    @staticmethod
    def await_api_request(action, config=None, node=None, run_delay=0, internal=False, interval=2, tries=10):
        """
        ...
        action_cmd = Cluster.api_request(action, config, node, run_delay, internal)
    ...
    def api_request(action, config=None, node=None, run_delay=0, internal=False):
        ...
        for node in [node] if node else Node.objects.exclude(management_ip__exact=''):
            ...
            if run_delay:
                ...
                m.result = f"Starting in {run_delay} seconds."
                m.save()
