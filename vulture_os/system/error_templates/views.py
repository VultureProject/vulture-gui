    ...
    if request.method == "POST" and form.is_valid():
        ...
        if object_id:
            ...
            if template.frontend_set.count() > 0:
                ...
                """ And reload HAProxy """
                for node in nodes:
                    api_res = node.api_request("services.haproxy.haproxy.reload_service", run_delay=settings.SERVICE_RESTART_DELAYttings.SERVICE_RESTART_DELAYttings.SERVICE_RESTART_DELAY)
                    if not api_res.get('status'):
