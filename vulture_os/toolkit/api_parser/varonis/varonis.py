...
class VaronisParser(ApiParser):
    ...
    ALL_COLUMNS_STATS = ["EventStatistics.ByAccount.Name","EventStatistics.ByAccount.SidID","EventStatistics.ByAccount.Identity.ID","EventStatistics.ByAccount.Identity.Name","EventStatistics.ByAccount.Identity.Manager.ID","EventStatistics.ByAccount.Identity.Manager.Name","EventStatistics.DirectEventCount","EventStatistics.NestedEventCount"]
    ...
    def varonis_api_search_create(self, rows, query):
        ...
        if res.status_code != 201:
            ...
            raise  Exception("problem with search. Aborting")
        content = res.json()
        (e_rows, e_terminate) = (content[0]["location"], content[1]["location"])

        if not e_rows:
            raise Exception("search has not being created. Aborting.")
    ...
    def varonis_api_search_rows(self, rows):
        ...
        if res.status_code != 200:
            ...
            raise Exception("Error while retrieving rows from search")
        return res.json()
