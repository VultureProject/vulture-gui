...
class CiscoUmbrellaManagedOrgParser(ApiParser):
    ...
    def __init__(self, data):
        ...
        self.cisco_umbrella_managed_org_proxy = data["cisco_umbrella_managed_org_proxy"]
        self.cisco_umbrella_managed_org_dns = data["cisco_umbrella_managed_org_dns"]
    ...
    def _get_parent_token(self):
        ...
        auth = f"{self.cisco_umbrella_managed_org_api_key}:{self.cisco_umbrella_managed_org_secret_key}"
        ...
        if response.status_code != 200:
            raise CiscoUmbrellaManagedOrgAPIError(f"Error at Cisco-Umbrella API Call URL: {self.TOKEN_URL} Code: {response.status_code} Content: {response.content}")
        token = response.json()
    ...
    def _get_customer_token(self, customer_id):
        ...
        auth = f"{self.cisco_umbrella_managed_org_api_key}:{self.cisco_umbrella_managed_org_secret_key}"
        auth_b64 = base64.b64encode(bytes(auth, encoding="utf-8")).decode()
        headers.update({"Authorization": f"Basic {auth_b64}"})
        response = requests.get(
            url=self.TOKEN_URL,
            headers=headers,
            proxies=self.proxies
    ...
    def test(self):
        try:
            log_types = []
            if self.cisco_umbrella_managed_org_dns:
                log_types.append("dns")
            if self.cisco_umbrella_managed_org_proxy:
                log_types.append("proxy")
            customer_ids_available = self._get_customers_id()
            logger.info(f"[{__parser__}]:test: Available customers ids are {set(customer_ids_available)}", extra={'frontend': str(self.frontend)})
            ...
            for customer_id in customer_ids:
                ...
                for log_type in log_types:
                    response = self.get_logs(timezone.now()-timedelta(hours=1), timezone.now(), customer_id, log_type)
    ...
    def get_logs(self, since, to, customer_id, log_type, index=0):
        ...
        self.session = requests.Session()
        self.session.headers.update({'X-Umbrella-OrgID': customer_id})
        ...
        logger.info(f"[{__parser__}]:get_logs: Cisco-Umbrella query parameters : {payload}, headers : {self.session.headers}, log type : {log_type}",
                     extra={'frontend': str(self.frontend)})
        return self.__execute_query(url, payload)
    ...
    def execute(self):
        ...
        try:
            ...
            logger.error(f"[{__parser__}]:execute: could not get the list of organization ids: {e}", extra={'frontend': str(self.frontend)})
        ...
        for customer_id in customer_ids:
            if not self.cisco_umbrella_managed_org_customers_tokens.get(customer_id) or not self.cisco_umbrella_managed_org_customers_tokens[customer_id].get("expires_at") or not self.cisco_umbrella_managed_org_customers_tokens[customer_id].get("access_token") or (self.cisco_umbrella_managed_org_customers_tokens[customer_id]["expires_at"] - timedelta(seconds=10) > timezone.now()):
                try:
                    ...
                    logger.error(f"[{__parser__}]:execute: {e}", extra={'frontend': str(self.frontend)})
            access_token = self.cisco_umbrella_managed_org_customers_tokens[customer_id]["access_token"]
            ...
            for log_type in log_types:
                ...
                try:
                    logs = self.get_logs(customer_id, log_type, since, to)
                except Exception as e:
                    logger.error(f"[{__parser__}]:execute: {e}", extra={'frontend': str(self.frontend)})
                index = 0
                ...
                while logs_count == self.LIMIT_MAX and index <= self.OFFSET_MAX:
                    try:
                        response = self.get_logs(customer_id, log_type, since, to)
                    except Exception as e:
                        logger.error(f"[{__parser__}]:execute: {e}", extra={'frontend': str(self.frontend)})
                ...
                if logs_count == self.LIMIT_MAX and index == self.OFFSET_MAX + self.LIMIT_MAX:
                    ...
                    self.last_collected_timestamps[timestamp_field_name] = datetime.fromtimestamp(timestamp, tz=timezone.now().astimezone().tzinfo)
                else:
