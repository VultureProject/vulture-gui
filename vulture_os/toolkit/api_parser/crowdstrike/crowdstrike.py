...
class CrowdstrikeParser(ApiParser):
    ...
    def __execute_query(self, method, url, query, timeout=10):
        ...
        while(retry > 0):
            ...
            try:
                ...
                logger.info(f"[{__parser__}][__execute_query] Low-level response {response.json()}", extra={'frontend': str(self.frontend)})
            except requests.exceptions.ReadTimeout:
                time.sleep(timeout)
                continue
    ...
    def execute_query(self, method, url, query={}, timeout=10):
        ...
        # by default to -1 ensuring line 176 to not break on first page
