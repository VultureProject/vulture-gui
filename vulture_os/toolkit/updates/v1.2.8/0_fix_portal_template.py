    ...
    if not node:
        ...
        try:
            for template in PortalTemplate.objects.all():
                ...
                template.html_password = template.html_password.replace('<input type="text" class="form-control" name="{{reset_password_name}}" value="{{reset_password_key}}"/>', '{{reset_password_code}}', '<input type="hidden" class="form-control" name="{{reset_password_name}}" value="{{reset_password_key}}"/>')
                template.save()
