...
__doc__ = "Rebuild all Filebeat Frontend configurations"
    ...
    if not node:
        ...
        try:
            for frontend in Frontend.objects.filter(mode="filebeat"):
                ...
                print(f"Listener {frontend}({frontend.ruleset}) Filebeat configuration rebuild asked.")
            node.api_request("services.filebeat.filebeat.restart_service")

        except Exception as e:
            print("Failed to rebuild filebeat configurations: {}".format(e))
