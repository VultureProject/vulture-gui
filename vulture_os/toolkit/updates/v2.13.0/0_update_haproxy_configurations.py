...
if __name__ == "__main__":
    ...
    if not node:
        ...
        try:
            ...
            api_res = node.api_request("services.haproxy.haproxy.reload_service", run_delay=django.conf.settings.SERVICE_RESTART_DELAY)
            if not api_res.get('status'):
                print("API error while trying to "
                            "restart HAProxy service : {}".format(api_res.get('message')))

        except Exception as e:
            print("Failed to update some Haproxy configurations: {}".format(e))
