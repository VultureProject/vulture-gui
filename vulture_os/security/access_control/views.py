...
from security.access_control.form import AccessControlForm, AccessControlRuleForm
...
def access_control_get(request):
    try:
        ...
        if log_id:
            client = MongoBase()
            log_line = client.execute_request("logs", "haproxy", {'id': log_id}, first=True)
            log_line['id'] = str(log_line['id'])
