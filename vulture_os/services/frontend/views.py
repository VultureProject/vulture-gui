...
def frontend_delete(request, object_id, api=False):
    ...
    if not used_by and (api or (request.method == "POST" and request.POST.get('confirm', "").lower() == "yes")):
        ...
        haproxy_filename = frontend.get_base_filename() if frontend.has_haproxy_conf else ""

        # Save rsyslog frontend conf filename
        # Whatever the type, delete the file because its name is its id
        rsyslog_filename = frontend.get_rsyslog_base_filename() if frontend.has_rsyslog_conf else ""

        # Save filebeat conf filename
        # Whatever the type, delete the file because its name is its id
        filebeat_filename = frontend.get_filebeat_base_filename() if frontend.has_filebeat_conf else ""
...
def frontend_edit(request, object_id=None, api=False):
    ...
    if request.method in ("POST", "PUT"):
        ...
        try:
            ...
            for node in old_nodes:
                if old_rsyslog_filename:
                    ...
                    or node not in new_nodes:
                        ...
                        node.api_request('services.rsyslogd.rsyslog.build_conf', run_delay=settings.SERVICE_RESTART_DELAY)
                        # API request to ensure timezone rulesets are up-to-date on node
                        node.api_request('services.rsyslogd.rsyslog.configure_timezones', run_delay=settings.SERVICE_RESTART_DELAY)
            ...
            Cluster.api_request("services.pf.pf.service_reload", run_delay=settings.SERVICE_RESTART_DELAY)
